...
class Net_SC(nn.Module):
    def __init__(self):
            ...
            self.C2 = nn.Sequential(
                _CrossNeuronBlock(256, 27, 1, spatial_height=16, spatial_width=16, reduction=16),
                nn.BatchNorm2d(256),
                nn.ReLU(True)
            )
...
LR = 0.01
net = Net_SC().cuda()
opt = Adam_GC(net.parameters(),lr=LR,weight_decay=1e-7)
...
# np.save('Store/Oppotunity/epoch_c3.npy',epoch_list)
# np.save('Store/Oppotunity/accuracy_c3.npy',accuracy_list)
# np.save('Store/Oppotunity/loss_c3.npy',loss_list)
